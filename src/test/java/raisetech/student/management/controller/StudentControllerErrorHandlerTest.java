...
class StudentControllerErrorHandlerTest extends ControllerTestBase {
   ...
   *
   * <ul>
   *   <li>{@code converter.decodeUuidStringOrThrow(studentId)} が UUIDの受講生IDを返す
   *   <li>受講生・コース変換 {@code converter.toEntity(...)} および {@code converter.toEntityList(...)}
   *       は正常に完了する
   *   <li>{@code service.updateStudentWithCourses(...)} が {@link ResourceNotFoundException} を送出する
   * </ul>
   *
   * <p>When:
   *
   * <ul>
   *   <li>MockMvc で {@code PUT /api/students/{studentId}} を実行する
   * </ul>
   *
   * <p>Then:
   *
  ...
  void updateStudent_存在しない受講生IDを指定した場合_404を返すこと() throws Exception {
    ...
    when(converter.toEntityList(anyList(), eq(studentId))).thenReturn(List.of());
   ...
   *   <li>想定外例外発生のため、{@code StudentConverter} を用いたレスポンス組み立て IDエンコード／3引数版 {@code toDetailDto(...)}
   *       等が呼び出されないことを検証する
   * </ul>
   *
   * @throws Exception MockMvc 実行時の例外
   ...
   * <ul>
   *   <li>HTTP ステータス 404 が返る
   *   <li>{@code status}=404, {@code code}={@code "E404"}, {@code error}={@code "NOT_FOUND"}
   *       がレスポンスに含まれる
   ...
   */
  @Test
  public void partialUpdateStudent_該当する受講生情報が存在しない場合_404を返すこと() throws Exception {
   ...
   *   <li>想定外例外発生のため {@code updateStudentInfoOnly(...)} や {@code searchCoursesByStudentId(...)}
   *       が呼び出されないことを検証する
   * </ul>
   *
   * @throws Exception MockMvc 実行時の例外
  ...
  public void partialUpdateStudent_想定外の例外が発生した場合_500を返すこと() throws Exception {
    ...
    // Controllerの事前チェックに引っかからない "NON_EMPTY" なボディにする
    // （student の中身は薄くてOK）
    String body = """
        {
          "student": { "fullName": "x" }
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.error").value("INTERNAL_SERVER_ERROR"))
        .andExpect(jsonPath("$.message").value(containsString("エラーが発生しました")));
   ...
   * <ul>
   *   <li>HTTP ステータス 500 が返る
   *   <li>レスポンスボディの {@code code} が {@code "E999"}、{@code error} が {@code "INTERNAL_SERVER_ERROR"}
   *       である
   *   <li>{@code softDeleteStudent} 以外に余計なサービス呼び出しが行われないことを検証する
   * </ul>
   *
   ...
   */
  @Test
  public void restoreStudent_対象受講生が存在しないまたは未削除の場合_404を返すこと() throws Exception {
