 ...
 *
 * <p>{@link ControllerTestBase} を継承し、MockMvc と @MockBean の StudentService / StudentConverter
 * を共有して利用します。
 */
class AdminStudentControllerTest extends ControllerTestBase {
  ...
  void forceDeleteStudent_正常系_204が返ること() throws Exception {
    ...
    // when & then
    mockMvc.perform(delete("/admin/students/{studentId}", idStr)).andExpect(status().isNoContent());
  ...
  void forceDeleteStudent_UUID形式として不正なIDを指定した場合_400が返ること() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.error").value("INVALID_ID_FORMAT")) // ここも仕様にあわせて
        .andExpect(jsonPath("$.message").value(Matchers.containsString("UUID")));
