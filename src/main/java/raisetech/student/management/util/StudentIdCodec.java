 ...
 *
 * <p>Bean 名を {@code "idCodec"} に固定することで、 既存コードの {@code @Qualifier("idCodec")} との互換性を維持します。
 */
@Component("idCodec")
public class StudentIdCodec implements IdCodec {
   ...
   *
   * <p>内部的には {@link #decodeUuidOrThrow(String)} で UUID にデコードしたうえで、 その最上位ビット／下位ビットを 16
   * バイト配列として詰め直します。
   *
   * @param uuidString UUID 文字列表現（例: {@code 123e4567-e89b-12d3-a456-426614174000}）。 {@code null}
   *     は許可されません。
   ...
   *
   * <p>{@link UUID#randomUUID()} でランダムな UUID を採番し、 {@link UUIDUtil#toBytes(UUID)} を使って 16
   * バイト配列に変換して返します。
   *
   * <p>主キーの新規採番など、「まだ ID を持たないエンティティ」に対して 利用することを想定しています。常に長さ 16 の配列を返し、 {@code null} を返すことはありません。
   *
