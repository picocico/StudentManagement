 ...
 *
 * <p><b>【 @WebMvcTest における意図的なセキュリティ無効化 】</b> {@code @WebMvcTest} (コントローラー層の単体テスト) を実行する際、
 * 認証・認可のロジックがテストの妨げになることがあります（例：401/403の発生）。
 *
 * <p>{@code @Order(0)} と {@code securityMatcher("/**")} を指定することで、 メインの {@code SecurityConfig}
 * よりも先にこの設定が適用され、"test" プロファイルが 有効な場合は<b>すべてのエンドポイントに対する認証・認可を無効化</b>します。
 *
 * <p>これにより、{@link raisetech.student.management.controller.StudentController} や {@link
 * raisetech.student.management.controller.admin.AdminStudentController} の
