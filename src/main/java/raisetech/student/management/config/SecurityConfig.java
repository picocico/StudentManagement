 ...
 *
 * <p>本設定では、物理削除API（/api/admin/**）へのアクセスに認証を要求し、 その他のAPIは認証不要としています。また、Basic認証とCSRF無効化を明示的に設定します。
 */
...
public class SecurityConfig {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http.authorizeHttpRequests(
            auth ->
                auth
                    // ★ここを authenticated() → hasRole("ADMIN") に
   ...
   *
   * <p>簡易的にインメモリで"admin"ユーザーを定義しています。
   *
